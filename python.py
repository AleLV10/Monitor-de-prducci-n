...
def actualiza(usuario, contraseña):
    ...
    connect = Coneccion.coneccion(host='192.168.128.234', database='C:\\Users\\RH1\\Downloads\\COPAC 2024.FDB', user=usuario, password=contraseña, consulta=consulta)
    if connect== "Conexión exitosa a Firebird!":
        if ban:  # Verificar si la pantalla ya está en ejecución
            pantalla()
    else:
...
def tabla_timer(tree):
    while True:
        ...
        # Extraer los datos relevantes para la gráfica
        etiquetas = [item[1] for item in Coneccion.lista if item[1] is not None and not "bobina" in item[1].lower()]
        cantidades_faltantes = [round(float(item[4]), 2) for item in Coneccion.lista if item[4] is not None and not "bobina" in item[1].lower()]
        ...
        # Extraer los datos relevantes para la gráfica
        etiquetas2 = [item[1] for item in Coneccion.lista if item[1] is not None and "bobina" in item[1].lower()]
        cantidades_faltantes2 = [item[4] for item in Coneccion.lista if item[4] is not None and "bobina" in item[1].lower()]
...
def pantalla():
    ...
    menu_Producción.add_command(label="Descargar PDF", command=lambda: enviar_correo(generar_pdf()))
    ...
    label_turno.place(relx=0.15, rely=0.01, relwidth=0.15, relheight=0.11)
    ...
    label_monitor.place(relx=0.31, rely=0.01, relwidth=0.50, relheight=0.11)
    ...
    # Crear el lienzo de matplotlib dentro de Tkinter
    canvas.draw()
    canvas.get_tk_widget().place(relx=0.01, rely=0.50, relwidth=0.30, relheight=0.49)
    ...
    # Crear el lienzo de matplotlib dentro de Tkinter
    canvas2.draw()
    canvas2.get_tk_widget().place(relx=0.32, rely=0.50, relwidth=0.32, relheight=0.49)


    label_turno_anterior = ttk.Label(root, text="   "+turno(1)+"   ", font=("Helvetica", 20,"bold underline"), anchor="center")
    label_turno_anterior .place(relx=0.65, rely=0.11, relwidth=0.34, relheight=0.10)
